function Hm = updateH(Xmis, Bm, Hm, gcell, type, lambda, parallel)
% function to conduct update for  latent factor matrix H under missing values.
% Bm = hBm; Hm = hHm;
%----------------------------------------------------------------------------------
% Author:       Liu Wei
...
if parallel
    ...
    %         Bng = Bng + B(gcell{j},:)'* diag(OdQ(i,gcell{j}))*B(gcell{j},:);
    %     case 'poisson'
    %         Bng = Bng + B(gcell{j},:)'* diag(mucell{j}(i,:).* OdQ(i,gcell{j})) * B(gcell{j},:);
    ...
    %         Bng = Bng + (B(gcell{j},:))' * diag(mucell{j}(i,:).*(1-mucell{j}(i,:)).* OdQ(i,gcell{j})) * B(gcell{j},:);
    % 
    %         end
    %    end
    %    d2f{i} = Bng + lambda* eye(q,q);
