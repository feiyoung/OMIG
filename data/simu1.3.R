# # Ex3. mix of normal, Poisson and binomial------------------------------------------
setwd("./")
source("./Dfunctions.R")


# Given q -----------------------------------------------------------------
## Screen ex3.1 run: jj=1, 2 and jme=1(MCAR)
## Screen ex3.2 run: jj=1,2 and jme=2(MAR)

library(missMDA)
misrateList<- list(seq(0.2-0.1, 0.2+0.1,by=0.1), seq(0.4-0.1, 0.4+0.1, by=0.1))
jj <- 2
q <- 5
n <- 200; p <- 200; type1 = 'npb'
rho <- 8
...
for(i in 1:N){
  ...
  id_bino <- which(group==3)
  for(j in id_bino) Xdat[!is.na(Xdat[,j]),j] <- as.factor(Xdat[!is.na(Xdat[,j]),j])
  out <- try({
...
errMat[1:3,,]
colMeans(errMat)
stmp <- paste0('./Rdata_R1/AE_FixQsimu3_misRate',jj,"mech", mecha_set[jme])
...
for(i in 1:N){
  ...
  if(class(out) == 'try-error'){
    ...
    out <- try({
      misList <- gfmImpute(dat$Xmis, group, type, q=hq, epsLogLike=1e-4,
                           maxIter=20,verbose=1, parallel=F)
      hX <- misList$hX
      mind<- is.na(dat$Xmis)
      AE(hX, dat$X, dat$Xmis, group)
    }, silent = T)
  ...
  #stmp <- paste0('./Rdata_R1/AE_chooseQsimu3_misRate',jj,"mech", mecha_set[jme])
