...
void VB_Estep(const field<mat>& Xf, const sp_mat& A, const vec& typeID, mat& Mu_y,  mat& S_y,
  ...
  for(i=0; i<d; ++i){ // update Mu_y and S_y by the variable types, note the missing values.
      ...
      if(typeID(i)==3){ // Binomial
        ...
        Mu_y2 = (Xf(i)- (1/(1+exp(-Mu_y2))) % repmat(nvec,n, 1) + Mu_y2%(1/(1+exp(-Mu_y2))) % (1-(1/(1+exp(-Mu_y2)))) % repmat(nvec,n, 1) + invLamMat.cols(p_vec(i), p_vec(i+1)-1)% tZ.cols(p_vec(i), p_vec(i+1)-1)) /
          ( (1/(1+exp(-Mu_y2))) %(1- 1/(1+exp(-Mu_y2)))  %(1- 1/(1+exp(-Mu_y2)))  % repmat(nvec,n, 1) + invLamMat.cols(p_vec(i), p_vec(i+1)-1) );
