---
title: 'OMIG: simulation for count and categorical variables'
author: "Wei Liu"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{OMIG: simulation for count and categorical variables}
...
We illustrate the application of `OMIG` to a simulated dataset containing mixed-type data, including both count and categorical variables by function `gendata()`.
...
## Simulate the mixed data with count and categorical variables

### Model setting and generate data
First, we set the model setting for generating the mixed-type data with count and categorical variables. The number of factors is set `q=4`, the variable types is set `type <- "pois_bino"`, the sample size of first batch is set `n=100`, the signal stregth is set by `rho=2`, the missing rates for three equal columns of matrix are set `mis_vec= c(0.1, 0.2, 0.3)`. Then, we generate the first data batch.
```{r  eval = FALSE}
q <- 4
type <- "pois_bino"
n <- 100; p <- 100; rho <- 2; mis_vec <- c(0.1, 0.2, 0.3)
dat <- gendata(q = q, n=n, p=p, type= type, rho=rho, mis_vec= mis_vec)
```
...
suppressWarnings(res <- OMIG(Xmis_new=dat$Xmis, q=q,  group=dat$group, type= types, hist.summary= NULL, lambda=lambda,  verbose=T))
```
...
We present a visual comparison of the results to evaluate the performance. In the first data batch, OMIG demonstrates equivalency to its offline counterpart, OrMIG. Consequently, the imputation error is nearly identical for both methods. Importantly, both OrMIG and OMIG outperform LFM-based methods. In this case, OrMIG with AM algorithm performs worse than that with VEM algorithm.

```{r  eval = FALSE}
df <- data.frame(NAE = c(err_omig, err_am, err_vem, err_jms, err_xp), Method=rep(c("OMIG", "AM", "VEM", "LFM-EM", "LFM-PR"), each=2), Type = rep(c("Count", "Binary"), length=10))
...
First, We apply the Eigevalue ratio test to select the number of factors. We found this method works well and is able to select the correct one.
```{r  eval = FALSE}
hq <- selectFacNumber(dat$Xmis, group=group, types=types)
print(c(q_est=hq, q_true=q))
```
Second, we apply the information criterion to select the number of factors. We found this method works well and is also able to select the correct one.
```{r  eval = FALSE}
...
```{r  eval = FALSE}
n2 <- 40 
dat <- gendata(q = q, n=n2, p=p, type= type, rho=rho, mis_vec= mis_vec, seed=2)
```
...
```{r  eval = FALSE}
res2 <- OMIG(Xmis_new=dat$Xmis, q,  dat$group, types, hist.summary= res$hist.summary, lambda=1, verbose=TRUE)
err_omig <- NAE(res2$hX, dat$X, dat$Xmis, dat$group)
```

We utilize the offline version of OMIG by employing the `OrMIG()` function, which solely incorporates the current data batch for model fitting. Subsequently, we calculate the imputation errors. We found the AM algorithm is not stable for this small-sample-size data and produces error.
```{r  eval = FALSE}
res_mig <- OrMIG(dat$Xmis, dat$group, types, q, algorithm = "VEM", verbose = F)
err_vem <- NAE(res_mig$hX, dat$X,  dat$Xmis, dat$group)
out <- try(res_mig2 <- OrMIG(dat$Xmis, dat$group, types, q, algorithm = "AM", lambda=0.1 ,verbose = T), silent=T)
str(out)
err_am <- rep(NA, 2)
```
We visualize the results to compare the performance. Upon analyzing the second data batch, we observe that OMIG exhibits significantly lower imputation errors compared to its offline version, OrMIG, because OMIG use the information from the first data batch in the form of sumamry statistics. 
```{r  eval = FALSE}
df <- data.frame(NAE = c(err_omig, err_am, err_vem), Method=rep(c("OMIG", "AM", "VEM"), each=2), Type = rep(c("Count", "Binary"), length=6))
...
```{r  eval = FALSE}
n2 <- 50
dat <- gendata(q = q, n=n2, p=p, type= type, rho=rho, mis_vec= mis_vec, seed=100)
```
...
res_mig <- OrMIG(dat$Xmis, dat$group, types, q, algorithm = "VEM", verbose = F)
err_vem <- NAE(res_mig$hX, dat$X,  dat$Xmis, dat$group)
suppressWarnings(res_mig2 <- OrMIG(dat$Xmis, dat$group, types, q, algorithm = "AM", lambda = 1, verbose = F))
err_am <-NAE(res_mig2$hX, dat$X,  dat$Xmis, dat$group)
```
We present a visual comparison of the results to assess performance. Upon analyzing the second data batch, we observe that OMIG demonstrates significantly lower imputation errors compared to its offline version, OrMIG. This improvement is attributed to OMIG's utilization of information from the first data batch in the form of summary statistics. Additionally, we note that OrMIG with the AM algorithm produces unstable results, with notably large Normalized Absolute Error (NAE) for count variables. Therefore, we recommend employing the offline version of OMIG with the VEM algorithm to impute missing values when using the offline version. Alternatively, for larger sample-size data batches, one may opt to use the offline version with the AM algorithm for imputation.
```{r  eval = FALSE}
df <- data.frame(NAE = c(err_omig, err_am, err_vem), Method=rep(c("OMIG", "AM", "VEM"), each=2), Type = rep(c("Count", "Binary"), length=6))
df$Method <- factor(df$Method, levels=c("OMIG", "AM", "VEM"))
ggplot(df, aes(x=Type, y=NAE, fill=Method)) + geom_bar(position = "dodge", stat="identity",width = 0.5)
